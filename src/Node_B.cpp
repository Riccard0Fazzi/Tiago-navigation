...
// import to change pose of the thiago's head
...
class TiagoAction{ 
	...
	public:
		...
		TiagoAction(std::string name):as_(nh_, name, boost::bind(&TiagoAction::tiagoActionCB, this, _1), false), action_name_(name)
		{
			...
			tag_sub = nh_.subscribe("/tag_detections", 100, &TiagoAction::tagDetectionCallback, this);
			head_pub = nh_.advertise<trajectory_msgs::JointTrajectory>("/head_controller/command", 10);
		}
	...
	private:
		...
		// Publisher for head joint control
		ros::Publisher head_pub;
		// subscriber for laser scanner topic
		ros::Subscriber laser_scan_sub;
		// subscriber for apriltag detection
		ros::Subscriber tag_sub;
		// subscriber to see the pictures from the camera mounted on tiago
        ...
        // simple behaviour ...
        geometry_msgs::Twist behaviorOne(int i){
	    ...
	    if(i < 25){lin_vel = 1.0; ang_vel = 0;}
	    else lin_vel = 0;
	    //if(i > 35 && i < 47){lin_vel = 0; ang_vel = 1.0;}
	    //if(i > 50){lin_vel = 0; ang_vel = 0;}
            //define the vector3 linear velocity parameters
            tf::Vector3 next_linear_vel(lin_vel, 0, 0);
            ...
            //define the vector3 angular velocity parameters
             tf::Vector3 next_angular_vel(0, 0, -ang_vel);
...
*/
	    ...
	    int i = 0;
	    int adjusted_head_iterations = 0;
		...
		if(adjusted_head_iterations < 17){
			ROS_WARN("Try to adjust head!!");
		    	// the next lines are to adjust the pose of the thiago's head
		    	trajectory_msgs::JointTrajectory head_cmd;
		    	head_cmd.joint_names = {"head_1_joint", "head_2_joint"}; // Pan and Tilt joints
		    	// Define the desired positions for pan and tilt
		    	trajectory_msgs::JointTrajectoryPoint point;
		    	point.positions = {0.0, -1.0}; // Pan = for horizontal, Tilt = for vertical (up/down)
		    	point.time_from_start = ros::Duration(1.0); // 1 second to achieve the goal
		    	head_cmd.points.push_back(point);
		    	// Publish the command
		    	head_pub.publish(head_cmd);
			adjusted_head_iterations++; // increase the counter to adjust the head
			rate.sleep();
			continue; // Restart loop at the next iteration
			// this continue command is needed because in this way thiago adjust the camera angle before starting to move
		}
                // activate BEHAVIORAL CONTROL
                // this control manages different simple behaviours of Tiago to explore the environment
                behavioralControl(i);
		i++;
